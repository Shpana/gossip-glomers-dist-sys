...
namespace tasks::gcounter {
  namespace {
    constexpr State::Clock::duration cache_invalidation_timeout{1s};
  }// namespace
  ...
  ReadHandler::read(maelstrom::Network::Session& session) {
...

    {
      ...
      for (auto& id: env_->available_node_ids) {
        ...
        // Sequantial consistent systems may not provide last value for reading without such calls
...

    {
      ...
      value = std::accumulate(
          results.begin(), results.end(), 0ull,
