...
namespace ds::broadcast {
  ...
  DistributorWorker::process(maelstrom::Network::Session&& session) {
    ...
    std::vector<yaclib::FutureOn<maelstrom::Unit>> futs;
    ...
    return yaclib::WhenAll(futs.begin(), futs.end())
        .ThenInline([](auto&& result) { return maelstrom::Unit{}; });
  }
}// namespace ds::broadcast