...
namespace ds::core::detail {
  ...
  NetworkProcessor::NetworkProcessor(Transport& transport)
      : transport_{transport} {}

  void NetworkProcessor::start() { std::exchange(is_running_, true); }
  ...
  NetworkProcessor::callOnceInternal(Request&& request,
...

    {
      ...
      once_.emplace(id, Waiter<WaitPolicy::Once>{.p = std::move(p),
                                                 .deadline = deadline});
    }
