...
namespace ds::core::detail {
  ...
  public:
    WorkersProcessor(yaclib::IExecutor& executor, Network& network);
  ...
  private:
    ...
    std::unordered_map<std::string, std::shared_ptr<WorkerBase<State>>>
        workers_{};
  };
  ...
  void WorkersProcessor<State>::add(Args&&... args) {
    ...
    auto worker = std::make_shared<Worker>(std::forward<Args>(args)...);
  ...
  void WorkersProcessor<State>::process() {
    ...
    for (auto [type, worker]: workers_) {
      auto deadline = worker->next_deadline_;
      worker->next_deadline_ = now + worker->period_;
      next_deadline = std::min(next_deadline, worker->next_deadline_);

      auto guess = ExecutionState::Idle;
      if (deadline < now && worker->exec_state_.compare_exchange_strong(
